...
| 数学 | 数论基础 | `math-gcd-lcm`（GCD与LCM·欧几里得算法）, `math-sieve-eratosthenes`（埃氏筛）, `math-euler-phi`（欧拉函数）, `math-linear-sieve-divisor-count`（线性筛求约数个数）, `math-linear-sieve-divisor-sum`（线性筛求约数和） |
